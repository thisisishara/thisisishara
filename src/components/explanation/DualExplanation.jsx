...
class DualExplanation extends React.Component {
  ...
  render() {
    let dualScores = Object.keys(this.state.data.dual[0].dual.probability_scores).map((key, index) =>
      <ExplanationBar key={key} sliderValue={Number((Number(this.state.data.dual[0].dual.probability_scores[key]) * 100).toFixed(2)).toString() + "%"} token={key.toString()} color={this.state.color} />
    );
    return (
      <Box className="mb-5">
        <Typography variant="h6" gutterBottom component="div" sx={{ fontWeight: 'medium' }} >Dual Feature Importance</Typography>
        <Box className="mb-4">
        ...
        <Box className="">
          <Accordion className="explanation-accordian explanation-accordian-dual">
            ...
            <AccordionDetails>
              <TwoColDataTable data={this.props.data.dual[0].dual.feature_importance} classNames="app-table-dual" />
            </AccordionDetails>
          </Accordion>
          {/* <Accordion>
                        <AccordionSummary
