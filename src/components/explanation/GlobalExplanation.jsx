...
class GlobalExplanation extends React.Component {
    ...
    render() {
        let globalScores = Object.keys(this.state.data.dual[0].global.probability_scores).map((key, index) =>
            <ExplanationBar key={key} sliderValue={Number((Number(this.state.data.dual[0].global.probability_scores[key]) * 100).toFixed(2)).toString() + "%"} token={key.toString()} color={this.state.color} />
            // <ExplanationBar sliderValue="20%" token="SLIIT" color={this.state.color} />
            // <ExplanationBar sliderValue="10%" token="උපාධි" color={this.state.color} />
            // <ExplanationBar sliderValue="20.98%" token="මොනවද" color={this.state.color} />
        );
        ...
        return (
            <Box className="mb-5">
                <Typography variant="h6" gutterBottom component="div" sx={{ fontWeight: 'medium' }} >Global Feature Importance</Typography>
                <Box className="mb-4">
                ...
                <Box className="mb-5">
                    <Accordion className="explanation-accordian explanation-accordian-global">
                        ...
                        <AccordionDetails>
                            <TwoColDataTable data={this.props.data.dual[0].global.feature_importance} classNames="app-table-global" />
                        </AccordionDetails>
                    </Accordion>
                    <Accordion className="explanation-accordian explanation-accordian-global">
                        <AccordionSummary
                        ...
                        <AccordionDetails>
                            <TwoColDataTable data={featureData} classNames="app-table-global" />
                        </AccordionDetails>
                    </Accordion>
                </Box>
            </Box>
