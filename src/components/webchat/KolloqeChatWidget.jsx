...
export default class KolloqeChatWidget extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      sessionFound: false,
      messageQueue: [],
      selectedLanguage: localStorage.getItem(`${localStorageKeys.kolloqeSelectedLang}`) || this.props.defaultLang,
  ...
  componentDidMount() {
    ...
    socket.on('bot_uttered', (response) => {
      ...
      if (response?.table) {
        this.displayTable(response.table);
      }
    });
  }
  ...
  handleLangSwitch(event) {
    ...
    localStorage.setItem(
      `${localStorageKeys.kolloqeSelectedLang}`,
      language
    )
    this.setState({
      selectedLanguage: language,
    }, () => {
      this.messageRef.handleLanguage(language);
      this.messageRef.handleInputFocus();
    });
  }
  ...
  displayTable(table) {
    this.displayMessage({ origin: "bot", type: 'table', content: table });
  }
  ...
  render() {
    ...
    function widgetPosition(position) {
      if (position === "left") {
        return {
          bottom: "10px",
          left: "101px"
      ...
      this.state.socketId !== undefined || !this.props.hideWhenNotConnected ?
        <Box
          sx={{
            ...
            position: 'fixed',
            zIndex: 'modal',
            ...widgetPosition(this.props.position),
          }}>
          <Stack
            direction={'column'}
                          ...
                          <Stack
                            ...
                            <Stack
                              ...
                              {(this.props.widgetOptions && this.props?.widgetOptionsPosition === "top") &&
                                <>
                                  {this.props.langSwitch &&
                                    <ThemeProvider theme={swithTheme}>
                                      <Tooltip title="Switch Language" placement="bottom">
                                        <FormControlLabel
                                          control={
                                            <Switch
                                              checked={this.state.selectedLanguage === "en" ? false : true}
                                              onChange={this.handleLangSwitch}
                                              className={"kolloqe-lang-switch"} />
                                          }
                                          label={this.state.selectedLanguage === "en" ? "En" : "Si"}
                                          onChange={this.handleLangSwitch}
                                          className={`kolloqe-lang-switch-label ${this.state.selectedLanguage === "en" ? "kolloqe-lang-switch-en" : "kolloqe-lang-si"}`} />
                                      </Tooltip>
                                    </ThemeProvider>
                                  }
                                  {this.props.fullscreenButton &&
                          ...
                          <Box
                            ...
                            <Stack
                              ...
                              {this.props.langSwitch &&
                                <ThemeProvider theme={swithTheme}>
                                  {this.props.langSwitch &&
                                    <Tooltip title="Switch Language" placement="top-start">
                                      <FormControlLabel
                                        control={
                                          <Switch
                                            checked={this.state.selectedLanguage === "en" ? false : true}
